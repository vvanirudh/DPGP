...
function twoagents_plotTrajs(trajs, string_in)
    ...
    num_cluster = max(max(trajs.cluster(:,:, end)));
    
    % trajs with cluster assignment 0 are singleton clusters
    ifRemove = 1 - min(min(trajs.cluster(:, :, end)));
    num_figure = ceil(num_cluster / num_group_per_subplot) + ...
        ifRemove;
    ...
    for k=1:num_figure
        ...
        for i=1:n_trajs
            clus_assign = trajs.cluster(i, 1, end);
            if (clus_assign > low_bnd) && (clus_assign <= high_bnd)
                c = colors{mod(trajs.cluster(i,1,end),7)+1};
                plot(trajs.data1(i).x, trajs.data1(i).y, c);
                hold on
                plot(trajs.data1(i).x(1), trajs.data1(i).y(1), strcat(c,'*'));
            end
            clus_assign = trajs.cluster(i, 2, end);
            if (clus_assign > low_bnd) && (clus_assign <= high_bnd)
                c = colors{mod(trajs.cluster(i,2, end),7)+1};
                plot(trajs.data2(i).x, trajs.data2(i).y, c);
                hold on
                plot(trajs.data2(i).x(1), trajs.data2(i).y(1), strcat(c,'o'));
            end
        end

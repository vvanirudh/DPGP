function [likelihood l_vector]= DP_traj_likelihood_indep(sparseGP_x, sparseGP_y, traj)

n = length(traj.x);

[mu_x_vec, var_x_matrix] = sparseGP_predict(sparseGP_x, traj.x, traj.y);
[mu_y_vec, var_y_matrix] = sparseGP_predict(sparseGP_y, traj.x, traj.y);
...
if (nargout == 2)
   l_vector = zeros(n,1);
       for i = 1:n
            ...
            ly = -0.5 * log(var_y) - 0.5 * y_diff^2 /var_y;
            l_vector(i) = lx;
            %fprintf(sprintf('i: %d, x_pred %f, x_act %f, x_diff %f, var_x %f, y_pred %f, y_act %f, y_diff %f, var_y %f\n', ...
            %    i, mu_x, traj.dx_dt(i), x_diff, var_x, mu_y, traj.dy_dt(i), y_diff, var_y));
       end
end
